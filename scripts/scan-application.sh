...
Arguments:
    APPLICATION_NAME    Name of application from config (required if env var APP_NAME not set)
...
load_app_config() {
    ...
    local app_index
    app_index=$(yq eval ".applications | map(.name) | to_entries | .[] | select(.value == \"$app_name\") | .key" "$CONFIG_FILE")
    ...
    if [ -z "$app_index" ] || [ "$app_index" = "null" ]; then
        ...
        yq eval '.applications[].name' "$CONFIG_FILE" | sed 's/^/  - /'
        return 1
    fi
    
    # Extract application configuration
    local config_namespace config_selector config_group config_tier
    config_namespace=$(yq eval ".applications[$app_index].namespace" "$CONFIG_FILE")
    config_selector=$(yq eval ".applications[$app_index].labelSelector" "$CONFIG_FILE")
    config_group=$(yq eval ".applications[$app_index].projectGroup" "$CONFIG_FILE")
    config_tier=$(yq eval ".applications[$app_index].projectTier // 3" "$CONFIG_FILE")
